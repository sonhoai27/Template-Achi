import * as React from "react";
import ClientHeader from "../client-shared/Header";
import Footer from "../client-shared/Footer";

class ClientSourceHome extends React.Component {
  constructor(props) {
    super(props);
  }
  render() {
    return (
      <>
        <ClientHeader/>
        <div className="col-xs-12 page-source">
          <div className="row page-source_banner">
            <div className="container">
              <div className="row">
                <div className="col-sm-6">
                  <h1 className="white" style={{ marginBottom: 64 }}>
                    <b>
                      FROM A HISTORY-MAKING STANLEY CUP FINALS TO BUILDING A
                      PERSONAL AI
                    </b>
                  </h1>
                  <p className="white">
                    Where has Tony been recently – and what in the world is he
                    up to? Below is the latest edition of the Tony Tracker,
                    where you can see highlights from his journeys and read his
                    narration of some of his favorite experiences. Watching
                    history being made at the Stanley Cup Finals TONIGHT’S GAME
                    IS ONE
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="row page-source_list-source">
            <div className="container paddingY-128">
              <div className="row">
                <div className="col-xs-12 page-source_title">
                  <h2 style={{}}>
                    <b>Có phí</b>
                  </h2>
                  <hr style={{ background: "#444" }} />
                </div>
                <div className="col-sm-4">
                  <div className="panel sales-page-listing">
                    <img
                      className="thumb"
                      src="https://kajabi-storefronts-production.global.ssl.fastly.net/kajabi-storefronts-production/site/10/images/xQtMXUjkR52oNAxgxzEF_HPMP_new.png"
                    />
                    <div className="panel-body">
                      <h3 className="title text-dark">
                        <a href="">High Performance Academy Master's Course</a>
                      </h3>
                      <p className="desc">
                        Advanced online training +&nbsp;personal development
                        coaching program: Reclaim your&nbsp;vibrancy, purpose +
                        success!
                      </p>
                      <a
                        className="btn btn-primary learn-more"
                        href="/store/mpSe8ToF"
                        kjb-settings-id="language_learn_more"
                      >
                        Learn More
                      </a>
                    </div>
                  </div>
                </div>
                <div className="col-sm-4">
                  <div className="panel sales-page-listing">
                    <img
                      className="thumb"
                      src="https://kajabi-storefronts-production.global.ssl.fastly.net/kajabi-storefronts-production/site/10/images/xQtMXUjkR52oNAxgxzEF_HPMP_new.png"
                    />
                    <div className="panel-body">
                      <h3 className="title text-dark">
                        <a href="">High Performance Academy Master's Course</a>
                      </h3>
                      <p className="desc">
                        Advanced online training +&nbsp;personal development
                        coaching program: Reclaim your&nbsp;vibrancy, purpose +
                        success!
                      </p>
                      <a
                        className="btn btn-primary learn-more"
                        href="/store/mpSe8ToF"
                        kjb-settings-id="language_learn_more"
                      >
                        Learn More
                      </a>
                    </div>
                  </div>
                </div>
                <div className="col-sm-4">
                  <div className="panel sales-page-listing">
                    <img
                      className="thumb"
                      src="https://kajabi-storefronts-production.global.ssl.fastly.net/kajabi-storefronts-production/site/10/images/xQtMXUjkR52oNAxgxzEF_HPMP_new.png"
                    />
                    <div className="panel-body">
                      <h3 className="title text-dark">
                        <a href="">High Performance Academy Master's Course</a>
                      </h3>
                      <p className="desc">
                        Advanced online training +&nbsp;personal development
                        coaching program: Reclaim your&nbsp;vibrancy, purpose +
                        success!
                      </p>
                      <a
                        className="btn btn-primary learn-more"
                        href="/store/mpSe8ToF"
                        kjb-settings-id="language_learn_more"
                      >
                        Learn More
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="row">
                <div className="col-xs-12 page-source_title">
                  <h2>
                    <b>Hội thảo</b>
                  </h2>
                  <hr style={{ background: "#444" }} />
                </div>
                <div className="col-sm-4">
                  <div className="panel sales-page-listing">
                    <img
                      className="thumb"
                      src="https://kajabi-storefronts-production.global.ssl.fastly.net/kajabi-storefronts-production/site/10/images/xQtMXUjkR52oNAxgxzEF_HPMP_new.png"
                    />
                    <div className="panel-body">
                      <h3 className="title text-dark">
                        <a href="">High Performance Academy Master's Course</a>
                      </h3>
                      <p className="desc">
                        Advanced online training +&nbsp;personal development
                        coaching program: Reclaim your&nbsp;vibrancy, purpose +
                        success!
                      </p>
                      <a
                        className="btn btn-primary learn-more"
                        href="/store/mpSe8ToF"
                        kjb-settings-id="language_learn_more"
                      >
                        Learn More
                      </a>
                    </div>
                  </div>
                </div>
                <div className="col-sm-4">
                  <div className="panel sales-page-listing">
                    <img
                      className="thumb"
                      src="https://kajabi-storefronts-production.global.ssl.fastly.net/kajabi-storefronts-production/site/10/images/xQtMXUjkR52oNAxgxzEF_HPMP_new.png"
                    />
                    <div className="panel-body">
                      <h3 className="title text-dark">
                        <a href="">High Performance Academy Master's Course</a>
                      </h3>
                      <p className="desc">
                        Advanced online training +&nbsp;personal development
                        coaching program: Reclaim your&nbsp;vibrancy, purpose +
                        success!
                      </p>
                      <a
                        className="btn btn-primary learn-more"
                        href="/store/mpSe8ToF"
                        kjb-settings-id="language_learn_more"
                      >
                        Learn More
                      </a>
                    </div>
                  </div>
                </div>
                <div className="col-sm-4">
                  <div className="panel sales-page-listing">
                    <img
                      className="thumb"
                      src="https://kajabi-storefronts-production.global.ssl.fastly.net/kajabi-storefronts-production/site/10/images/xQtMXUjkR52oNAxgxzEF_HPMP_new.png"
                    />
                    <div className="panel-body">
                      <h3 className="title text-dark">
                        <a href="">High Performance Academy Master's Course</a>
                      </h3>
                      <p className="desc">
                        Advanced online training +&nbsp;personal development
                        coaching program: Reclaim your&nbsp;vibrancy, purpose +
                        success!
                      </p>
                      <a
                        className="btn btn-primary learn-more"
                        href="/store/mpSe8ToF"
                        kjb-settings-id="language_learn_more"
                      >
                        Learn More
                      </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <Footer/>
      </>
    );
  }
}

export default ClientSourceHome;
